...
# 운영자 생성
1. 운영자 우클릭 후 새 운영자 클릭 -> 이름(운영자 이름), 전자메일이름(gmail주소), 호출기 전자메일이름(gmail주소)입력, 호출기 연락 가능 근무 일정(날짜)설정 -> 확인

# SQL Server Agent 알림 설정
1. SQL Servcer 에이전트 클릭 -> 작업 클릭 -> 설정할 작업 더블 클릭 -> 알림 -> 작업 완료시 수행할 동작 선택 후 하위 내용 선택(전자메일 등..) -> 확인